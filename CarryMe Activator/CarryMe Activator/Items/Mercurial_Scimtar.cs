...
{
	internal class Mercurial_Scimitar
	{
		public static ItemId Id = ItemId.Mercurial_Scimitar;
		...
		private void GameOnUpdate(EventArgs args)
		{
			if (ObjectManager.Player.HasItem(Id))
			{
				...
				var any = (from buddy in EntityManager.Heroes.Allies.Where(CleanserItems.IsActiveForHero) let buffs = GetBuffs(buddy) let longestBufftime = buffs.Select(buff => buff.EndTime - buff.StartTime).Concat(new[] {0f}).Max() where longestBufftime*1000 >= Menu[IdentBase + ".Duration"].Cast<Slider>().CurrentValue && buffs.Count() >= Menu[IdentBase + ".minBuffsforUse"].Cast<Slider>().CurrentValue select buddy).Any(buddy => buddy.IsMe);
				if (!any) return;
				Item.UseItem(Id);
			}
