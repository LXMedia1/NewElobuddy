...
{
	internal class aa_CleanserItem
	{
					...
					Buff.BuffList.Where(b => (b.DoT || b.Cleanse) && b.MenuName != null).OrderBy(b => !b.DoT).ThenBy(b => b.MenuName))
			{
				try
				{
					if (buff.ChampionName != null)
						if (!(EntityManager.Heroes.Enemies.Any(u => buff.ChampionName == u.ChampionName)))
							continue;
				...
				if (DelayTick + Menu[IdentBase + ".CheckDelay"].Cast<Slider>().CurrentValue < Core.GameTickCount)
				{
					...
					foreach (var buddy in EntityManager.Heroes.Allies.Where(CleanserItems.IsActiveForHero))
					{
						...
						if (!(longestBufftime >= Menu[IdentBase + ".Duration"].Cast<Slider>().CurrentValue) ||
						    buffs.Count() < Menu[IdentBase + ".minBuffsforUse"].Cast<Slider>().CurrentValue) continue;
						if (buddy.IsMe)
						{
							Item.UseItem(Id);
							return;
